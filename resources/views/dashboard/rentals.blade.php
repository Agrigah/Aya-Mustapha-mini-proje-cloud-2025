             ...
             <tbody>
                @foreach ($rentals as $rental)
                    <tr>
                        ...
                        <td scope="col">{{ $rental->user->name }}</td>
                        <td scope="col">{{ $rental->car->model }}</td>
                        <td scope="col">{{ Str::substr($rental->rental_date, 0, 10) }}</td>
                        <td scope="col">{{ Str::substr($rental->return_date, 0, 10) }}</td>
                        @if ($rental->status == '1')
                            <td scope="col">En attente d'approbation</td>
                            <td scope="col">
                                <form  action="{{ route('rental.status' , ['status' => '2' , 'id' => $rental->id])}}"method="POST">
                                    @csrf
                                    @method('PUT')
                        ...
                        @elseif ( $rental->status == '2' )
                            ...
                            <td scope="col">
                                <form method="POST" action="{{ route('rental.status' , ['status' => '3', 'id' => $rental->id])}}">
                                    @csrf
                                    @method('PUT')
                        ...
                        @elseif ( $rental->status == '3' )
                            ...
                            <td scope="col">
                                <form action="{{ route('rental.delete' , ['id' => $rental->id])}}" method="POST">
                                    @csrf
                                    @method('DELETE')
