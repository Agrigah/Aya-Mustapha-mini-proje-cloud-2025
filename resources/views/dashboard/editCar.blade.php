...
<section class="dashboard d-flex">
  <div class="tables w-25 h-100" style="background-color: #010530; color: white; py-5 d-flex flex-column;">
    <a href="{{ route('dashboard.users') }}" class="d-flex align-items-center justify-content-around border mb-3 text-dark text-bg-light text-decoration-none">
    ...
    <div class="w-75 h-100 d-flex  justify-content-center details">
        <form class="row h-75 m-5 p-5 text-bg-light" method="POST" action="{{ route('cars.update' , ['id' => $car->id ]) }}">
            @csrf
            @method('PUT')
            <div class="col-md-4">
              <label for="validationDefault01" class="form-label">Designation</label>
              <input type="text" class="form-control" id="validationDefault01" name="brand" value={{ $car->brand }} required />
            </div>
            <div class="col-md-4">
              <label for="validationDefault02" class="form-label">Model</label>
              <input type="text" class="form-control" id="validationDefault02" name="model" value={{ $car->model }} required />
            </div>
            <div class="col-md-4">
              <label for="validationDefaultUsername" class="form-label">Année d'apparution</label>
              <div class="input-group">
                <input type="text" class="form-control" id="validationDefaultUsername" aria-describedby="inputGroupPrepend2" name="year" value={{ $car->year }} required />
              </div>
            </div>
            <div class="col-md-6">
              <label for="validationDefault03" class="form-label">Images</label>
              <input type="text" class="form-control" id="validationDefault03" required name="images" value={{ $car->images }} />
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3">
              ...
              <input type="text" class="form-control" id="validationDefault05" required name="registration_number" value={{ $car->registration_number }} />
            </div>
            <div class="col-12">
                <label for="floatingTextarea2" class="mb-1">Description</label>
                <textarea class="form-control" id="floatingTextarea2" style="height: 100px" name="description">{{ $car->description }}</textarea>
              </div>
            <div class="col-12">
